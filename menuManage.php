						...
						<table class="table table-bordered table-hover mb-0">
                            ...
                            <?php
                                ...
                                while($row = mysqli_fetch_assoc($result)){
                                    ...
                                    $BagDesc = $row['BagDesc'];
                                    $BagCategorieId = $row['BagCategorieId'];

                                    echo '<tr>
                                            <td class="text-center">' .$BagCategorieId. '</td>
                                            <td>
                                                <img src="/LUXURY BAGS/img/pizza-'.$BagID. '.png" alt="image for this item" width="150px" height="150px">
                                            </td>
                                            <td>
                                                <p>Name : <b>' .$BagName. '</b></p>
                                                <p>Description : <b class="truncate">' .$BagDesc. '</b></p>
                                                <p>Price : <b>' .$BagPrice. '</b></p>
                                            </td>
                                            <td class="text-center">
												<div class="row mx-auto" style="width:112px">
													<button class="btn btn-sm btn-primary" type="button" data-toggle="modal" data-target="#updateItem' .$BagID. '">Edit</button>
...
<?php 
    $bagsql = "SELECT * FROM `bag`";
    $bagResult = mysqli_query($conn, $bagsql);
    while($bagRow = mysqli_fetch_assoc($bagResult)){
        $BagID = $bagRow['BagID'];
        $BagName = $bagRow['BagName'];
        $BagPrice = $bagRow['BagPrice'];
        $BagCategorieId = $bagRow['BagCategorieId'];
        $BagDesc = $bagRow['BagDesc'];
?>

<!-- Modal -->
<div class="modal fade" id="updateItem<?php echo $BagID; ?>" tabindex="-1" role="dialog" aria-labelledby="updateItem<?php echo $BagID; ?>" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header" style="background-color: rgb(111 202 203);">
        <h5 class="modal-title" id="updateItem<?php echo $BagID; ?>">Item Id: <?php echo $BagID; ?></h5>
	  	...
	  	<form action="partials/_menuManage.php" method="post" enctype="multipart/form-data">
		    <div class="text-left my-2 row" style="border-bottom: 2px solid #dee2e6;">
		   		<div class="form-group col-md-8">
					...
					<input type="hidden" id="BagID" name="BagID" value="<?php echo $BagID; ?>">
					<button type="submit" class="btn btn-success my-1" name="updateItemPhoto">Update Img</button>
				</div>
				<div class="form-group col-md-4">
					<img src="/LUXURY BAGS/img/pizza-<?php echo $BagID; ?>.jpg" id="itemPhoto" name="itemPhoto" alt="item image" width="100" height="100">
				</div>
			</div>
		</form>
		<form action="partials/_menuManage.php" method="post">
            <div class="text-left my-2">
                ...
                <input class="form-control" id="name" name="name" value="<?php echo $BagName; ?>" type="text" required>
            </div>
			<div class="text-left my-2 row">
				<div class="form-group col-md-6">
                	<b><label for="price">Price</label></b>
                	<input class="form-control" id="price" name="price" value="<?php echo $BagPrice; ?>" type="number" min="1" required>
				</div>
				<div class="form-group col-md-6">
					<b><label for="catId">Category Id</label></b>
                	<input class="form-control" id="catId" name="catId" value="<?php echo $BagCategorieId; ?>" type="number" min="1" required>
				</div>
            </div>
            <div class="text-left my-2">
                <b><label for="desc">Description</label></b>
                <textarea class="form-control" id="desc" name="desc" rows="2" required minlength="6"><?php echo $BagDesc; ?></textarea>
            </div>
            <input type="hidden" id="BagID" name="BagID" value="<?php echo $BagID; ?>">
