...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['addToCart'])) {
        ...
        $existSql = "SELECT * FROM `viewcart` WHERE BagID = '$itemId' AND `userId`='$userId'";
        ...
        if($numExistRows > 0){
            ...
            $sql = "INSERT INTO `viewcart` (`BagID`, `itemQuantity`, `userId`, `addedDate`) VALUES ('$itemId', '1', '$userId', current_timestamp())";   
    ...
    if(isset($_POST['removeItem'])) {
        $itemId = $_POST["itemId"];
        $sql = "DELETE FROM `viewcart` WHERE `BagID`='$itemId' AND `userId`='$userId'";   
        $result = mysqli_query($conn, $sql);
        ...
        if (password_verify($password, $passRow['password'])){ 
            ...
            if ($result){
                ...
                while($addrow = mysqli_fetch_assoc($addResult)){
                    ...
                    $itemSql = "INSERT INTO `orderitems` (`orderId`, `BagID`, `itemQuantity`) VALUES ('$orderId', '$BagID', '$itemQuantity')";
                    $itemResult = mysqli_query($conn, $itemSql);
                }
                $deletesql = "DELETE FROM `viewcart` WHERE `userId`='$userId'";   
                $deleteresult = mysqli_query($conn, $deletesql);
                echo '<script>alert("Thanks for buying with us. Your order ID is. ' .$orderId. '.");
                    window.location.href="http://localhost/LUXURY BAGS/index.php";  
                    </script>';
                    exit();
            }
        } 
    ...
    if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
    {
        $BagID = $_POST['BagID'];
        $qty = $_POST['quantity'];
        $updatesql = "UPDATE `viewcart` SET `itemQuantity`='$qty' WHERE `BagID`='$BagID' AND `userId`='$userId'";
