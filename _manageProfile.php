<?php
    ...
    if (isset($_POST["updateProfilePic"])) {
        // Check if file upload field is not empty
        if (isset($_FILES["image"]) && !empty($_FILES["image"]["tmp_name"])) {
            ...
            echo '<script>alert("No file was uploaded. Please select an image to upload.");
                    window.history.back(1);
                </script>';
        }
    }
    ...
    if(isset($_POST["removeProfilePic"])){
        $filename = $_SERVER['DOCUMENT_ROOT']."/LUXURY BAGS/img/person-".$userId.".jpg";
